...
import { getWorkSettings, calculateCheckInStatus, getCurrentTime } from '../../../utils/attendanceUtils';
...
const AbsenMasukView: React.FC<AbsenMasukViewProps> = ({
  ...
  const submitAttendance = async () => {
    ...
    try {
      ...
      const checkInStatus = calculateCheckInStatus(currentTimeStr, workSettings);
      ...
      onAttendanceSubmit(newAttendance);
      setCapturedPhoto(null);
    } catch (error) {
    ...
    return (
      <div className="space-y-4 lg:space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 lg:p-8 text-center">
          ...
          <p className="text-sm lg:text-base text-gray-600 mb-4">
            Anda tidak dapat melakukan absensi masuk karena sedang dalam status izin yang disetujui
          </p>
          <button
            onClick={onBackToDashboard}
            className="px-4 lg:px-6 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors"
    ...
    return (
      <div className="space-y-4 lg:space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 lg:p-8 text-center">
          ...
          <p className="text-sm lg:text-base text-gray-600 mb-4">
            Absen masuk pada {todayAttendance.checkInTime} menggunakan {todayAttendance.checkInMethod === 'qr' ? 'QR Code' : 'Foto'}
          </p>
          <button
            onClick={onBackToDashboard}
            className="px-4 lg:px-6 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors"
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      ...
      {!isCapturing && !capturedPhoto && !cameraError && locationChecked && (
        <div className="bg-white rounded-xl shadow-lg p-6 lg:p-8 text-center">
          <LogIn className="w-16 h-16 lg:w-20 lg:h-20 mx-auto mb-4 lg:mb-6 text-blue-500" />
          <h3 className="text-xl lg:text-2xl font-bold text-gray-800 mb-3">Siap untuk Absen Masuk?</h3>
          ...
          {isWithinRadius ? (
            <button
              onClick={startCamera}
              className="px-8 lg:px-10 py-3 lg:py-4 bg-blue-500 text-white rounded-xl hover:bg-blue-600 transition-colors font-medium flex items-center space-x-3 mx-auto shadow-lg hover:shadow-xl transform hover:scale-105 text-sm lg:text-lg"
            >
